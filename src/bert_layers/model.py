...
class FlexBertForSequenceClassification(FlexBertPreTrainedModel):
    ...
    def from_composer(
        ...
        **kwargs,
        ...
        state_dict = torch.load(pretrained_checkpoint)
        # Handle full Composer checkpoint format (state['model'] nesting)
        if "state" in state_dict and "model" in state_dict["state"]:
            state_dict = state_dict["state"]["model"]
        # If the state_dict was saved after wrapping with `composer.HuggingFaceModel`, it takes on the `model` prefix
        consume_prefix_in_state_dict_if_present(state_dict, prefix="model.")
        
        # Get model's expected keys for validation
        model_keys = set(model.state_dict().keys())
        checkpoint_keys = set(state_dict.keys())
        
        missing_keys, unexpected_keys = model.load_state_dict(state_dict, strict=False)
        
        # Calculate how many weights were actually loaded
        loaded_keys = model_keys - set(missing_keys)
        num_loaded = len(loaded_keys)
        num_total = len(model_keys)
        
        # Check if essentially nothing was loaded (allow for classifier head to be missing)
        # Core model weights (bert.*) should be loaded
        core_missing = [k for k in missing_keys if k.startswith("bert.") and not k.endswith("position_embeddings.weight")]
        if num_loaded == 0 or len(core_missing) > num_total * 0.5:
            raise RuntimeError(
                f"ERROR: Failed to load pretrained weights from checkpoint!\n"
                f"Checkpoint: {pretrained_checkpoint}\n"
                f"Loaded: {num_loaded}/{num_total} weights\n"
                f"Core missing keys: {len(core_missing)}\n"
                f"This usually means the checkpoint format is incompatible.\n"
                f"Check if the checkpoint has the expected structure."
            )

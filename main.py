...
import src.arg_data as arg_data_module
...
def setup_logging(run_name: str = "default"):
    """Set up logging for the console only (with DEBUG level)"""
    ...
    # Optionally, set composer logger to DEBUG as well
    composer_logger = logging.getLogger('composer')
    composer_logger.setLevel(logging.DEBUG)


def log_config(cfg: DictConfig):
    """Enhanced config logging that writes to both file and wandb"""
...
def build_dataloader(
    ...
    if cfg.name == "text":
        ...
        data_loader = arg_data_module.build_arg_dataloader(
            cfg,
            tokenizer,
            device_batch_size,
            device_microbatch_size=device_microbatch_size,
...
def main(cfg: DictConfig, return_trainer: bool = False, do_train: bool = True) -> Optional[Trainer]:
    if dist.get_local_rank() == 0:  # Only setup logging on rank 0
